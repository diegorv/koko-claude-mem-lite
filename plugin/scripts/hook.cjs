...
var DATA_DIR = (0, import_path.join)((0, import_os.homedir)(), ".memory-lite");
function getDataDir() {
...
function getSettings() {
  ...
  if (!(0, import_fs2.existsSync)(path)) {
    (0, import_fs2.writeFileSync)(path, JSON.stringify(DEFAULTS, null, 2));
    cached = { ...DEFAULTS };
    return cached;
  }
  try {
    const raw = JSON.parse((0, import_fs2.readFileSync)(path, "utf-8"));
    cached = { ...DEFAULTS, ...raw };
    return cached;
  } catch {
    cached = { ...DEFAULTS };
...
async function handleContext(input) {
  ...
  if (!await ensureWorker()) {
    const pluginRoot = process.env.CLAUDE_PLUGIN_ROOT || (0, import_path2.dirname)(__dirname);
    const workerScript = (0, import_path2.join)(pluginRoot, "scripts", "worker.cjs");
    try {
...
async function handleSummarize(input) {
  ...
  if (input.transcriptPath) {
    try {
      const content = (0, import_fs3.readFileSync)(input.transcriptPath, "utf-8");
